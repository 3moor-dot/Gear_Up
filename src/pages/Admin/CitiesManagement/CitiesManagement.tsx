...
const CitiesManagement: React.FC = () => {
  ...
  return (
    <div className={`flex min-h-screen ${dark ? "bg-[#0F1323]" : "bg-white"} relative`}>
      {/* SIDEBAR */}
      <AdminSidebar />
      ...
      <main className="flex-1 px-10 pt-4">
        ...
        <div className="mb-8 text-right -mt-20"> 
          <h1 className={`text-3xl font-bold ${dark ? "text-white" : "text-[#0B2545]"}`}>
            إدارة المدن
          </h1>
        ...
        <div
          className={`rounded-2xl overflow-hidden transition-colors ${
            dark ? "bg-[#0B1020] border border-[#1E2A44]" : "bg-white border border-[#D6E9FF]"
          }`}
        >
          {/* TABLE HEADER */}
          <div
            className={`grid grid-cols-4 px-6 py-4 text-sm font-medium ${
              dark
          ...
          {/* ROW EXAMPLE */}
          <div
            className={`grid grid-cols-4 items-center px-6 py-4 text-sm transition ${
              dark ? "text-white hover:bg-[#111B34]" : "text-[#0B2545] hover:bg-[#F2F8FF]"
            }`}
          >
            <span>أوستن</span>
...
{isModalOpen && (
  ...
  >
    {/* modal  content */}
    <div
      className="w-[700px] rounded-3xl overflow-hidden shadow-2xl transition-all"
      dir="rtl"
      style={{ backgroundColor: dark ? "rgba(19,127,236,0.3)" : "rgba(19,127,236,0.3)" }}
    >
      {/* Header */}
  ...
  <h2 className="text-3xl font-bold text-white">إضافة مدينة جديدة</h2>
  <p className="text-white/70 text-sm mt-2">قم بإنشاء مدينة جديدة</p>
</div>
...
<div className="p-8 pt-0 flex gap-4 justify-end">
  <button
    onClick={() => setIsModalOpen(false)}
    className="w-[160px] py-4 rounded-xl text-white font-medium hover:bg-black/30 transition"
    style={{ backgroundColor: "#0F1323" }}
  >
    إلغاء
  </button>
  <button
    className="w-[160px] py-4 rounded-xl text-white font-medium hover:bg-blue-600 transition shadow-lg"
