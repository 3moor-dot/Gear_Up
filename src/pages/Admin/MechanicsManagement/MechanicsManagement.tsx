...
import {
  ...
  FaCheckCircle,
  FaTimesCircle,
  FaShieldAlt, // أيقونة جديدة للتحقق
} from "react-icons/fa";
import { useTheme } from "../../../contexts/ThemeContext";
...
interface ApiMechanicDetails {
  ...
  longitude: number;
  isAvailable: boolean;
  isVerified: boolean; // هذا الحقل مهم لنا
  supportsFieldVisit: boolean;
  workStartTime: string | null;
  workEndTime: string | null;
  createdAt: string;
...
const MechanicsManagement: React.FC = () => {
  ...
  // [NEW] دالة التحقق من المستند
  // const handleVerifyDocument = async () => {
  //   if (!selectedMechanic) return;

  //   // رسالة تأكيد قبل البدء (اختياري)
  //   if (
  //     !confirm(
  //       "هل تريد تشغيل الذكاء الاصطناعي للتحقق من رخصة الورشة؟\nإذا تمت الموافقة، سيتم تفعيل الحساب وتوثيقه تلقائياً."
  //     )
  //   ) {
  //     return;
  //   }
  ...
  //   params.append("mechanic_id", selectedMechanic.id);
  ...
  //         alert(
  //           "✅ تم التحقق بنجاح! المستند سليم.\nجاري تفعيل الحساب وتوثيق الميكانيكي..."
  //         );

  //         // 1. تحديث الواجهة المحلية فوراً (لتحسين التجربة)
  //         if (mechanicDetails) {
  //           setMechanicDetails({
  //             ...mechanicDetails,
  //             isVerified: true,
  ...
  //         // 2. استدعاء دالة التفعيل لتغيير الحالة من Pending إلى Active
  //         // هذا سيقوم أيضاً بتحديث القائمة وجلب البيانات مرة أخرى
  ...
  //           "المستند غير مقبول أو غير واضح.";
  //         alert(`⚠️ فشل التحقق: ${reason}`);
  //       }
  //     } else {
  //       alert("حدث خطأ أثناء الاتصال بخادم التحقق.");
  //     }
  //   } catch (error) {
  //     console.error("Verification Error:", error);
  //     alert("حدث خطأ غير متوقع أثناء عملية التحقق.");
  //   } finally {
  //     setIsVerifying(false);
  //   }
  // };
    // [NEW] دالة التحقق من المستند (محدثة)
   // [FINAL] دالة التحقق من المستند (معدلة لقراءة الخطأ المغلف)
   const handleVerifyDocument = async () => {
    if (!selectedMechanic) return;
    ...
    try {
      ...
      if (response.ok && result.success && result.data.isApproved) {
          ...
          if (result.data && result.data.message) {
            try {
              // السيرفر بيحط الرسالة داخل JSON String
              const parsedMsg = JSON.parse(result.data.message);
              if (parsedMsg.detail) {
                errorMessage = parsedMsg.detail;
              } else {
                errorMessage = result.data.message;
              }
            } catch  {
              // لو مش JSON، نعرضها زي ما هي
              errorMessage = result.data.message;
            }
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (response.ok) {
        ...
        // تحديث التفاصيل بعد تغيير الحالة
        await fetchMechanicDetails(selectedMechanic.id);
      } else {
        alert("فشل تحديث الحالة، يرجى المحاولة مرة أخرى");
      }
        ...
        <div className="p-5 space-y-5">
          ...
          {!loadingDetails && mechanicDetails && (
            <div className="space-y-5">
              ...
              {/* يظهر فقط إذا كان الحساب معلق (Pending) ولم يتم التوثيق بعد */}
              {mapStatus(mechanicDetails.accountStatus).status ===
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 mb-3">
                    ...
                    في حالة الموافقة التلقائية، سيتم تفعيل الحساب.
                  </p>
                  <button
