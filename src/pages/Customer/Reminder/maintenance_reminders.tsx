...
import { BsCalendarPlus } from "react-icons/bs";
import {
  FaTrash, FaCheck, FaPause, FaPlay, FaWrench, FaClock, FaSync, FaCalendarAlt ,   FaClipboardCheck
} from "react-icons/fa";
...
const MaintenanceReminders = () => {
  ...
  return (
     <div className="flex h-screen overflow-hidden dark:bg-primary_BGD" dir="rtl">
      ...
      {/* التعديل: نفس لون كروت التذكيرات (#137FEC33) */}
      <div 
        className={`fixed left-0 top-[90px] md:top-[210px] bottom-4 bg-white dark:bg-[#0f172acc] backdrop-blur-md shadow-2xl z-[60] transition-all duration-300 ease-in-out border-r border-slate-200 dark:border-slate-700 flex flex-col rounded-r-2xl overflow-hidden
        ${isCompletedDrawerOpen ? 'w-[85vw] sm:w-[320px]' : 'w-[42px] sm:w-[56px]'}`}
        dir="rtl"
      >
        {/* <div 
          className="h-20 flex flex-col  px-4 border-b border-slate-100 dark:border-blue-500/30 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors gap-1" */}
          <div 
  className="h-16 sm:h-20 flex flex-col items-center justify-center px-2 sm:px-4 border-b border-slate-100 dark:border-blue-500/30 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors gap-1"
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {/* <FaClipboardCheck className="text-blue-400 text-[18px]" /> */}
...
dark:border-slate-700
  ...
  : "متوقفة"}
                  ...
                  <select value={selectedCar} onChange={(e) => { setSelectedCar(e.target.value); localStorage.setItem("selectedCar", e.target.value); }} className={`bg-transparent font-bold text-xs w-full sm:w-auto outline-none cursor-pointer px-2 py-1 ${dark ? "text-white" : "text-slate-800"}`}>
                    {cars.map((car, idx) => (
                      <option key={idx} value={`${car.year} ${car.brand} ${car.model}`}>{car.year} {car.brand} {car.model}</option>
                    ))}
                  </select>
              ...
              <div className="space-y-6 max-h-[700px] overflow-y-auto overflow-x-hidden pr-2 pl-4 custom-scroll pb-4">
                {filteredActive.length > 0 ? filteredActive.map((r) => {
                  ...
                  return (
                  <div key={r.id} className="p-4 sm:p-6 md:p-4 rounded-2xl sm:rounded-[2.5rem] shadow-lg border transition-all dark:bg-[#0f172acc] dark:border-slate-600 border-slate-200 hover:shadow-xl md:hover:scale-105 transform-gpu duration-300">
                    <div className="flex flex-col sm:flex-row justify-between gap-4 sm:gap-6 items-start sm:items-center">
                      ...
                      <h1 className="text-lg sm:text-2xl font-black text-slate-700 dark:text-slate-200">{r.name}</h1>
                          {r.description && <p className="text-sm text-slate-500 dark:text-slate-400 font-medium italic">"{r.description}"</p>}
                        </div>
                      </div>
                      <div className="grid grid-cols-2 md:flex md:flex-col gap-3 text-xs font-bold border-r-2 border-slate-200 dark:border-slate-600 pr-0 md:pr-6 min-w-[150px]">
                    ...
                    {/* <div className="flex flex-col sm:flex-row flex-wrap gap-2 mt-5 pt-4 border-t border-slate-200 dark:border-slate-600 px-2">
                      {r.status === "Active" && (
                        <button onClick={() => handleStatusAction(r.id, "complete")} className="bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300 px-5 py-1.5 rounded-xl text-[11px] font-semibold flex items-center gap-2 hover:bg-emerald-200 transition">
                          <FaCheck size={11} /> إتمام
                        </button>
...
<button
  onClick={() =>
    handleStatusAction(r.id, r.status === "Active" ? "pause" : "activate")
  }
  className="
    w-full sm:w-auto
    sm:min-w-[120px]
