...
import { 
  ...
  FaWrench, FaClock, FaSync, FaHistory, FaCalendarAlt
} from "react-icons/fa";
            ...
            <div className="lg:col-span-3 space-y-6 order-2">
              <div className="bg-white dark:bg-[#137FEC33] p-6 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-600">
                <h3 className="font-bold text-xl mb-6 flex items-center gap-2">
                  <FaHistory className="text-blue-500"/> تاريخ مكتمل
              ...
              <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-2">
                ...
                <div className="flex items-center gap-3 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl shadow-inner">
                  ...
                  <select
                    ...
                    className={`bg-transparent font-bold text-sm md:text-base outline-none cursor-pointer px-3 py-2 ${dark ? "text-white" : "text-slate-800"}`}
                  >
                    {cars.map((car, idx) => (
                    ...
                    <div key={r.id} className="p-4 md:p-2 rounded-[2.5rem] shadow-sm border transition-all dark:bg-[#137FEC33] border-slate-200 dark:border-slate-600">
                      ...
                      <div className="flex flex-wrap gap-2 mt-5 pt-4 border-t border-slate-200 dark:border-slate-600 px-2">
                        {!isOnce && r.status === "Active" && (
                          <button onClick={() => handleStatusAction(r.id, "complete")} className="bg-emerald-500 text-white px-5 py-1.5 rounded-xl text-[11px] font-semibold flex items-center gap-2 hover:bg-emerald-600 transition">
                        ...
                        <button onClick={() => deleteReminder(r.id)} className="bg-red-100 text-red-600 dark:bg-red-500/10 dark:text-red-400 px-5 py-1.5 rounded-xl text-[11px] font-semibold flex items-center gap-2 hover:bg-red-200 dark:hover:bg-red-500/20 transition">
                          <FaTrash size={10}/> حذف
                        </button>
                      </div>
                    </div>
