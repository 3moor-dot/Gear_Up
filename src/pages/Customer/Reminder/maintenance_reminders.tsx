...
const isValidEndDate = (date?: string) => {
  if (!date) return false;

  // لو السيرفر بيرجع قيمة افتراضية
  if (date.includes("9999")) return false;
...
const MaintenanceReminders = () => {
  ...
  const handleStatusAction = async (id: number | string, action: string) => {
    try {
      // هذا الـ Endpoint يطابق ما طلبته بالضبط: POST /api/Reminder/{id}/complete
...
<div className="
  ...
  pr-0 md:pr-6
  min-w-full sm:min-w-[260px]
">

  {/* العمود الأول */}
  <div className="flex items-center gap-2 text-slate-700 dark:text-slate-200">
    <FaClock className="text-blue-500 shrink-0" />
    <span className="whitespace-nowrap">وقت التذكير:</span>
  ...
  {/* العمود الثاني */}
  <div className="flex items-center gap-2 text-slate-700 dark:text-slate-200">
    <FaSync className="text-blue-500 shrink-0" />
    <span className="whitespace-nowrap">نوع التكرار:</span>
    <span className="truncate">{getFrequencyLabel(r)}</span>
  </div>
  ...
  <div className="flex items-center gap-2 text-slate-700 dark:text-slate-200">
    ...
    <span className="leading-relaxed">
      تاريخ البداية: {formatToEgyptDate(r.startDate || "")}
    </span>
  </div>
  ...
  : "—"}
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 mt-5 pt-4 border-t border-slate-200 dark:border-slate-600 px-2">
                        ...
                        <button
                          onClick={() => {
                            setDeleteTargetId(r.id || r.reminderId || "");
                            setShowConfirm(true);
                          }}
                          className="
                            w-full sm:w-auto
