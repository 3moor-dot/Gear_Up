...
// // --- Types ---
// // تعديل مهم: إضافة carId و car object عشان نتأكد نلقي الـ ID في أي مكان
// type ServiceRequest = {
//   requestId: string;
...
//   status: string;
//   serviceType: string;
//   carId?: string | number; // ممكن يكون رقم أو نص
//   car?: {
//     id?: string | number;
//     brand?: string;
//     model?: string;
...
// type Car = {
//   id: string | number; // خليه يقبل نص أو رقم عشان الـ API
//   brand?: string;
//   model?: string;
//   year?: number;
//   plateNumber?: string;
...
//   // --- تعديل 1: استخدام نفس endpoint صفحة السجل عشان الـ IDs تكون موحدة ---
//   useEffect(() => {
...
//   // --- تعديل 2: منطق الفلترة القوي والذكي ---
...
//       // لو مفيش سيارة مختارة (فشل التحميل مثلاً)، ما نعرض حاجة
//       if (!currentCar) return false;

//       // 2. فلترة السيارة
//       // نحول كل الـ IDs لـ Strings عشان نتأكد من المطابقة (Type safety)
//       const currentCarIdStr = String(currentCar.id).trim();
      
//       // الحالة أ: الـ ID موجود مباشر في الطلب (carId)
//       if (item.carId) {
//         return String(item.carId).trim() === currentCarIdStr;
//       }

//       // الحالة ب: الـ ID موجود داخل كائن السيارة (car.id)
//       if (item.car?.id) {
//         return String(item.car.id).trim() === currentCarIdStr;
//       }

//       // لو ما لقناش ID أصلن، بنحاول نطابق بالاسم (كحل طوارئ)
//       if (item.car?.brand && item.car?.model && currentCar.brand && currentCar.model) {
//          return `${item.car.brand} ${item.car.model}` === `${currentCar.brand} ${currentCar.model}`;
//       }
...
//         <main className="flex-1 overflow-hidden p-4 md:p-8 flex flex-col gap-6">
...
//             <div className="col-span-1 lg:col-span-8 flex flex-col gap-6 h-full overflow-hidden">
//               {/* كارت السيارة */}
...
//                 <div className="space-y-4 overflow-y-auto pr-1 max-h-[260px] min-h-0 custom-scrollbar">
//                   {mechanicsLoading ? (
...
//                     <p className="text-center text-gray-400 text-xs py-4">لا يوجد سجل خدمات لهذه السيارة</p>
//                   ) : (
                    ...
                    <div className="text-center md:text-right w-full md:w-auto">
                      ...
                      {cars[selectedCarIndex]?.plateNumber && (
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                          رقم اللوحة: {cars[selectedCarIndex].plateNumber}
                        </p>
                      )}
                      {cars.length > 1 && (
                        <button
            ...
            <div className="col-span-1 lg:col-span-4 flex flex-col gap-6 h-full min-h-0">
              ...
              <div className="bg-[#137FECE5] dark:bg-[#162240] rounded-[30px] p-6 text-white border border-blue-300 dark:border-blue-900/40 shadow-sm flex flex-col shrink-0">
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-2 text-center md:text-right shrink-0">
                  ...
                  <button
                    onClick={() => setShowAllMechanics(!showAllMechanics)}
                    className="text-xs text-white dark:text-[#137FEC] underline hover:text-gray-200 dark:hover:text-blue-400 transition-colors font-bold"
                  >
                    {showAllMechanics ? "عرض الأعلى" : "عرض الكل"}
                  </button>
                </div>
                ...
                <div className="space-y-4 overflow-y-auto pr-2 max-h-[185px] min-h-0 custom-scrollbar">
                  {mechanicsLoading ? (
                    ...
                    <div className="text-center py-6">
                      ...
                      <p className="text-xs text-white/60 font-medium">لا يوجد ميكانيكيين حالياً</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-[#137FEC1A] dark:bg-[#162240] rounded-[30px] p-6 border border-blue-100 dark:border-blue-900/40 shadow-sm flex flex-col shrink-0">
                ...
                <div className="space-y-4">
                  {loading ? (
                    ...
                    <p className="text-center text-gray-400 text-sm py-4">لا يوجد سجل خدمات لهذه السيارة</p>
                  ) : (
                    dashboardHistory.map((service) => (
                      <div
                        key={service.requestId}
