...
const ActionMenu = ({
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute top-0 left-full ml-1 w-48 bg-white dark:bg-[#1E293B] border dark:border-gray-700 rounded-lg shadow-xl z-[9999] overflow-hidden">
          {status === "Pending" ? (
            <>
              <button
            ...
            <>
              {!booking.rating ? (
                <button
                  onClick={() => { onRate(booking); onClose(); }}
                  className="flex items-center gap-2 w-full text-right px-4 py-2.5 text-sm text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors font-medium"
                >
                  <MdStar size={16} />
                  تقييم الخدمة
                </button>
              ) : (
                <span className="flex items-center gap-2 w-full text-right px-4 py-2.5 text-sm text-green-600 dark:text-green-400 font-medium cursor-default">
...
const MaintenanceBookings = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const mapped: Booking[] = data.map((b) => {
        ...
        switch (b.status) {
          case "Pending":
            statusColor = "bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300";
            break;
          case "Confirmed":
          case "Accepted":
          ...
          case "Completed":
            ...
            break;
          default:
            statusColor = "bg-gray-100 text-gray-700 dark:bg-gray-600/20 dark:text-gray-300";
        }
        return {
          ...b,
  ...
  useEffect(() => {
    ...
    if (!data) return;
    if (data.mechanics?.[0]?.id) setSelectedMechanic(data.mechanics[0].id);
    if (data.service) setSelectedService(data.service);
    if (data.carId) setSelectedCar(data.carId);
    if (data.autoOpen) setIsModalOpen(true);
  }, [location.state]);
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (responseText?.includes("already")) message = "تم إرسال تقييم لهذا الحجز من قبل.";
        else if (responseText?.includes("Unauthorized")) message = "انتهت الجلسة، سجلي دخول مرة تانية.";
        else if (responseText?.includes("BookingRatings")) message = "حصلت مشكلة من السيرفر أثناء حفظ التقييم.";
      ...
      const message = err instanceof Error ? err.message : "حدث خطأ أثناء إرسال التقييم";
      Swal.fire({
        icon: "error",
        title: "فشل الإرسال",
        text: message,
  ...
  const filteredBookings = bookings
    .filter((booking) => {
      const statusMatch = selectedStatus === "الكل" || statusLabels[booking.status] === selectedStatus;
      const bookingDate = new Date(`${booking.date.split("T")[0]}T${booking.slotStart.slice(0, 5)}`);
      const now = new Date();
      let timeMatch = true;
      if (selectedTimeFilter === "اليوم") timeMatch = bookingDate.toDateString() === now.toDateString();
      ...
      if (selectedTimeFilter === "هذا الشهر") {
        timeMatch = bookingDate.getMonth() === now.getMonth() && bookingDate.getFullYear() === now.getFullYear();
      }
      return statusMatch && timeMatch;
    })
    .sort((a, b) => {
      const fullDateA = new Date(`${a.date.split("T")[0]}T${a.slotStart.slice(0, 5)}`).getTime();
      const fullDateB = new Date(`${b.date.split("T")[0]}T${b.slotStart.slice(0, 5)}`).getTime();
  ...
  return (
    <div className="flex h-screen overflow-hidden dark:bg-primary_BGD bg-gray-50" dir="rtl">
      <Sidebar />
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="text-right">
              ...
              <p className="text-[#94A3B8] text-sm mt-1">تتبع أعمال الصيانة والإصلاحات الخاصة بسيارتك</p>
            </div>
            <button
          ...
          {/* Filters Section */}
          <div className="bg-white dark:bg-[#0F172A] border dark:border-white/10 rounded-xl p-5 shadow-sm">
            <div className="flex flex-col md:flex-row gap-4">

              <div className="flex-1 w-full">
                <label className="text-gray-500 dark:text-gray-400 text-xs font-bold mb-2 block tracking-widest uppercase">
                  التوقيت
                </label>
                <div className="relative">
                  <select
                    value={selectedTimeFilter}
                    onChange={(e) => { setSelectedTimeFilter(e.target.value); setCurrentPage(1); }}
                    className="w-full appearance-none bg-gray-50 dark:bg-[#1E293B] text-gray-800 dark:text-white border border-gray-200 dark:border-[#334155] rounded-xl py-3 px-4 outline-none cursor-pointer text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                  >
          ...
          {/* Bookings List */}
          {loading ? (
            <div className="flex justify-center items-center min-h-[500px] w-full bg-white dark:bg-[#0F172A] rounded-xl border dark:border-white/5 shadow-sm">
              <div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" />
            </div>
          ) : filteredBookings.length === 0 ? (
            <div className="flex flex-col items-center justify-center min-h-[500px] w-full bg-white dark:bg-[#0F172A] rounded-xl border dark:border-white/5 shadow-sm">
              ...
              <p className="text-gray-600 dark:text-gray-300 font-semibold text-base">لا توجد حجوزات بهذه المعايير</p>
              <p className="text-gray-400 dark:text-gray-500 text-sm mt-1">جرب تغيير الفلاتر أو أضف حجزاً جديداً</p>
            </div>
          ) : (
            <>
              {/* Desktop Table */}
                  ...
                  <table className="w-full min-w-[900px]">
                    ...
                    <tbody>
                      {paginatedBookings.map((booking) => (
                        ...
                        >
                          <td className="p-4 font-medium text-gray-800 dark:text-white text-sm">{booking.mechanicName}</td>
                          <td className="p-4 text-gray-600 dark:text-gray-400 text-sm">{booking.subSpecializationName}</td>
                          <td className="p-4 text-gray-600 dark:text-gray-400 text-sm">{formatDisplayDate(booking.date)}</td>
                          <td className="p-4 text-gray-600 dark:text-gray-400 text-sm">{booking.time}</td>
                          <td className="p-4">
                            <span className={`px-2.5 py-0.5 rounded-md text-xs font-normal ${booking.statusColor}`}>
                          ...
                          <td className="p-4">
                            <ActionMenu
                              ...
                              onToggle={() => setOpenMenuId(openMenuId === booking.id ? null : booking.id)}
                              onClose={() => setOpenMenuId(null)}
                              onReschedule={(b: Booking) => { setSelectedBooking(b); setIsRescheduleOpen(true); }}
                              onCancel={(b: Booking) => { setSelectedBooking(b); setIsCancelModalOpen(true); }}
                ...
                {paginatedBookings.map((booking) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-bold text-gray-800 dark:text-white">{booking.mechanicName}</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">{booking.subSpecializationName}</p>
                      </div>
                      <span className={`px-2.5 py-1 rounded-md text-xs font-normal ${booking.statusColor}`}>
                        {statusLabels[booking.status] || booking.status}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center text-sm text-gray-600 dark:text-gray-300 border-t dark:border-gray-800 pt-3">
                      <div><span className="font-semibold ml-1">التاريخ:</span>{formatDisplayDate(booking.date)}</div>
                      <div><span className="font-semibold ml-1">الوقت:</span>{booking.time}</div>
                    </div>

                    {booking.status === "Pending" && (
                      <div className="flex gap-2 pt-3 border-t dark:border-gray-800">
                        <button
                          onClick={() => { setSelectedBooking(booking); setIsRescheduleOpen(true); }}
                      ...
                      <div className="pt-3 border-t dark:border-gray-800">
                        {!booking.rating ? (
                          ...
                          <div className="w-full bg-green-50 dark:bg-green-900/20 text-green-600 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2">
                            <MdCheckCircle size={18} /> تم التقييم
                          </div>
                        )}
                      </div>
                    )}
      ...
      >
        <div className="flex items-center justify-between p-6 border-b dark:border-gray-800">
          <h2 className="text-xl font-bold dark:text-white text-gray-800">تقييم الخدمة</h2>
          <button
            onClick={() => !ratingLoading && setIsRatingOpen(false)}
          ...
          <div className="p-6 space-y-6">
            {ratingSuccess ? (
              <div className="flex flex-col items-center justify-center py-16 space-y-4">
                ...
                <h3 className="text-xl font-bold dark:text-white text-gray-800">شكراً لتقييمك!</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center">تم إرسال تقييمك بنجاح، نقدر رأيك كثيراً</p>
              </div>
            ) : (
              <>
                ...
                <p className="text-center text-sm text-gray-500 dark:text-gray-400">كيف كانت تجربتك مع الخدمة؟</p>
                ...
                <div className="rounded-xl p-5 space-y-4 bg-gray-50 dark:bg-[#131c2f] border dark:border-gray-800">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-gray-800 dark:text-white text-sm truncate">{ratingBooking.mechanicName}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">ميكانيكي</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2.5 pt-2 border-t dark:border-gray-700">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium dark:text-white text-gray-800">{ratingBooking.subSpecializationName}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500 dark:text-gray-400">📅 التاريخ</span>
                      <span className="text-sm font-medium dark:text-white text-gray-800">{formatDisplayDate(ratingBooking.date)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500 dark:text-gray-400">⏰ التوقيت</span>
                      <span className="text-sm font-medium dark:text-white text-gray-800">{ratingBooking.time}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-bold text-gray-700 dark:text-gray-300 block">التعليق</label>
                  <textarea
                    value={comment}
                ...
                >
                  {ratingLoading ? (
                    ...
                    <><MdStar size={18} /> إرسال التقييم</>
                  )}
                </button>

                {stars === 0 && (
                  <p className="text-xs text-red-500 text-center -mt-3">يجب اختيار تقييم واحد على الأقل</p>
                )}
              </>
            )}
          </div>
      ...
      <AddBookingModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={() => fetchBookings()} />
      <RescheduleModal isOpen={isRescheduleOpen} booking={selectedBooking} onClose={() => setIsRescheduleOpen(false)} onSuccess={() => fetchBookings()} />
      <CancelBookingModal isOpen={isCancelModalOpen} booking={selectedBooking} token={token ?? undefined} onSuccess={() => fetchBookings()} onClose={() => setIsCancelModalOpen(false)} />
    </div>
  );
};
