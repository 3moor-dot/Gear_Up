...
//     return token ? `notifications_${token.slice(-10)}` : "guest_notifications";
//   }, [token]);
...
//   try {
//     const res = await axios.get(
//       `https://gearupapp.runasp.net/api/requests/${data.requestId}/status`,
//       { headers: { Authorization: `Bearer ${token}` } }
//     );

//     issue = res.data?.issueDescription || issue;

//     if (res.data?.mechanic) {
//       mechanicName = `${res.data.mechanic.firstName} ${res.data.mechanic.lastName}`;
//     }
...
//   const statusText = data.status; // أو استخدمي map لو عندك

//   const newNotification = {
//     title: "تم تحديث حالة الطلب 🔄",

//     message: `📌 الطلب: "${issue}"
// 🛠️ بواسطة: ${mechanicName}
...
// <div className="max-h-80 overflow-visible space-y-3 custom-scrollbar px-1">


// {notifications.length > 0 ? notifications.map((n, i) => (
//   <div
//   key={i}
//   className={`relative p-3.5 transition-all border-b last:border-b-0 ${
//     dark
...
//       className="flex-1 text-[11px] py-1 rounded bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white font-bold"
//     >
//       إتمام
//     </button>
...
//         }
//         className="w-full text-[11px] py-1 rounded bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500 hover:text-white font-bold"
//       >
//         تأجيل
//       </button>
...
//         <div className="absolute bottom-full mb-3 right-0 z-[99999] bg-white dark:bg-slate-800 shadow-2xl rounded-lg p-2 w-48 max-h-60 overflow-y-auto divide-y divide-gray-200 dark:divide-slate-600">
...
//               }
//               className="block w-full text-xs py-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded"
//             >
//               {opt.label}
//             </button>
//           ))}
