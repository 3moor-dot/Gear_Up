...
const Sidebar = () => {
  ...
  // جلب البيانات من السيرفر لضمان تحديث الصورة فوراً
  const fetchSidebarProfile = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUserData(data);
        // تحديث البيانات في التخزين المحلي أيضاً للمزامنة
        sessionStorage.setItem("userData", JSON.stringify(data));
      } else {
        // إذا فشل الطلب، استخدم البيانات الموجودة في sessionStorage كحل احتياطي
  ...
  useEffect(() => {
    fetchSidebarProfile();
  }, [location.pathname]); // إعادة الجلب عند التنقل بين الصفحات لضمان المزامنة
      ...
      <aside className={`fixed inset-y-0 right-0 z-40 w-72 dark:bg-primary_BGD bg-white h-screen flex flex-col shadow-2xl lg:shadow-none transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"} lg:translate-x-0 lg:static lg:block`} dir="rtl">
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto">
          {menuItems.map((item, index) => {
            ...
            return (
              <Link key={index} to={item.path} onClick={() => setIsOpen(false)} className={`flex items-center gap-4 px-6 py-3 rounded-xl transition-all ${isActive ? 'bg-[#E5F1FD] dark:bg-[#137FEC1A] text-[#137FEC] font-bold shadow-sm' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 hover:text-[#137FEC]'}`}>
        ...
        <div className="p-6 border-t border-gray-50 dark:border-gray-800">
          <div className="rounded-[25px] border border-blue-400/30 p-4 bg-gray-50/50 dark:bg-[#137FEC0D]">
            ...
            <div className="space-y-3">
              <Link to={settingsPath} onClick={() => setIsOpen(false)} className={`flex items-center gap-3 w-full px-4 py-2 rounded-xl text-sm ${isSettingsActive ? 'bg-[#E5F1FD] text-[#137FEC] font-bold' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100'}`}>
                <MdSettings className="text-xl" />
                <span>الاعدادات</span>
              </Link>
              <button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-2 text-red-500 hover:bg-red-50 rounded-xl text-sm">
